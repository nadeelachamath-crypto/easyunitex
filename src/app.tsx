...
export function convertBase(value: string, fromBase: number, toBase: number): string {
  ...
  if (isNaN(decimal)) throw new Error('Invalid characters for selected base')
...
const decodeUTF8 = (base64: string) => {
...
// ===================== UNIT DATA =====================
const linearConv = (factors: Record<string, number>) => (val: number, from: string, to: string) => {
  try {
    const v = math.bignumber(val);
    const f = math.bignumber(factors[from]);
    const t = math.bignumber(factors[to]);
    return Number(v.times(f).divide(t).toString());
  } catch {
...
const unitData: Record<string, { units: string[]; conversion: (val: number, from: string, to: string) => number }> = {
  length: {
    ...
    conversion: linearConv({ Meter: 1, Kilometer: 1000, Mile: 1609.344, Foot: 0.3048, Inch: 0.0254, Centimeter: 0.01, Millimeter: 0.001, Yard: 0.9144 })
  },
  weight: {
    units: ['Kilogram', 'Gram', 'Pound', 'Ounce', 'Milligram', 'Ton'],
    conversion: linearConv({ Kilogram: 1, Gram: 0.001, Pound: 0.45359237, Ounce: 0.028349523125, Milligram: 0.000001, Ton: 1000 })
  },
  temperature: {
  ...
  area: {
    ...
    conversion: linearConv({ 'Square Meter': 1, 'Square Kilometer': 1000000, 'Square Mile': 2589988.110336, 'Square Foot': 0.09290304, 'Acre': 4046.8564224, 'Hectare': 10000 })
  },
  volume: {
    units: ['Liter', 'Milliliter', 'Cubic Meter', 'Gallon', 'Quart', 'Pint', 'Cup'],
    conversion: linearConv({ 'Liter': 1, 'Milliliter': 0.001, 'Cubic Meter': 1000, 'Gallon': 3.785411784, 'Quart': 0.946352946, 'Pint': 0.473176473, 'Cup': 0.2365882365 })
  },
  'volume-dry': {
    units: ['Bushel (US)', 'Peck (US)', 'Dry Gallon (US)', 'Liter'],
    conversion: linearConv({ 'Bushel (US)': 35.23907, 'Peck (US)': 8.8097675, 'Dry Gallon (US)': 4.40488375, 'Liter': 1 })
  },
  time: {
  ...
  speed: {
    ...
    conversion: linearConv({ 'Meter per Second': 1, 'Kilometer per Hour': 0.2777777777777778, 'Mile per Hour': 0.44704, 'Knot': 0.5144444444444444, 'Foot per Second': 0.3048 })
  },
  pressure: {
    units: ['Pascal', 'Kilopascal', 'Bar', 'PSI', 'Atmosphere'],
    conversion: linearConv({ 'Pascal': 1, 'Kilopascal': 1000, 'Bar': 100000, 'PSI': 6894.75729, 'Atmosphere': 101325 })
  },
  energy: {
  ...
  power: {
    ...
    conversion: linearConv({ Watt: 1, Kilowatt: 1000, Horsepower: 745.699872, 'BTU/hr': 0.29307107 })
  },
  force: {
  ...
  numbers: {
    units: ['Decimal', 'Binary', 'Octal', 'Hexadecimal', 'Ternary', 'Quinary', 'Duodecimal', 'Base 36'],
    conversion: (val, from, to) => {
      const baseMap: Record<string, number> = {
        'Decimal': 10, 'Binary': 2, 'Octal': 8, 'Hexadecimal': 16,
        'Ternary': 3, 'Quinary': 5, 'Duodecimal': 12, 'Base 36': 36
      };
      return convertBase(val.toString(), baseMap[from], baseMap[to]);
    }
  },
  'velocity-angular': {
...
function ConversionResult({ label, value, copyable = true }: { label: string; value: string; copyable?: boolean }) {
  ...
  return (
    <div className="glass p-4 rounded-xl flex items-start justify-between gap-4">
      <div className="flex-1 min-w-0">
        <p className="text-xs uppercase tracking-wide text-gray-500">{label}</p>
        <p className="text-xl font-mono font-semibold break-all whitespace-pre-wrap">{value}</p>
      </div>
      {copyable && (
        <button onClick={handleCopy} className="p-2 rounded-lg hover:bg-white/40 dark:hover:bg-gray-800/40 shrink-0">
          {copied ? '✓' : '📋'}
        </button>
      )}
    </div>
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="glass p-6 rounded-2xl space-y-4">
        ...
        <div className="flex gap-4 items-center">
          <select value={from} onChange={e => setFrom(e.target.value)} className="glass dark:bg-gray-800/50 p-2 rounded-xl border-primary/30 text-gray-900 dark:text-gray-100 outline-none">
          ...
          <select value={to} onChange={e => setTo(e.target.value)} className="glass dark:bg-gray-800/50 p-2 rounded-xl border-primary/30 text-gray-900 dark:text-gray-100 outline-none">
            {currencies.map(c => <option key={c} value={c} className="text-black">{c}</option>)}
          </select>
        </div>
      </div>
        ...
        <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full glass dark:bg-gray-800/50 p-4 rounded-2xl border-primary/30 outline-none focus:border-primary appearance-none text-gray-900 dark:text-gray-100"
            >
              {defs.units.map(u => <option key={u} value={u} className="text-black">{u}</option>)}
            </select>
          </div>
      ...
      <div className="mt-8 p-6 bg-primary/5 rounded-2xl border border-primary/10 flex items-center justify-between">
        <div>
          ...
          <p className="text-4xl font-mono font-bold text-primary">
            {typeof result === 'number' ? result.toFixed(6).replace(/\.?0+$/, '') : result} <span className="text-lg font-normal text-gray-600">{toUnit}</span>
          </p>
        </div>
        <button
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8">
      ...
      <div className="glass p-6 rounded-2xl space-y-4">
        ...
        <div className="flex gap-4 items-center">
          <select value={fromUnit} onChange={e => setFromUnit(e.target.value)} className="glass dark:bg-gray-800/50 p-2 rounded-xl border-primary/30 text-gray-900 dark:text-gray-100 outline-none">
            {defs.units.map(u => <option key={u} className="text-black">{u}</option>)}
          </select>
          <span>→</span>
          <select value={toUnit} onChange={e => setToUnit(e.target.value)} className="glass dark:bg-gray-800/50 p-2 rounded-xl border-primary/30 text-gray-900 dark:text-gray-100 outline-none">
            {defs.units.map(u => <option key={u} className="text-black">{u}</option>)}
          </select>
        </div>
      </div>
...
function NumberSystemPage() {
  ...
  let steps = []
  try {
    result = convertBase(input, fromBase, toBase)
    steps = conversionSteps(input, fromBase, toBase)
  } catch (e: any) {
    result = e.message || 'Invalid input'
    steps = []
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="glass p-6 rounded-2xl space-y-4">
        ...
        <div className="flex gap-4 items-center">
          <div><label>From Base</label><select value={fromBase} onChange={e => setFromBase(+e.target.value)} className="w-full glass dark:bg-gray-800/50 p-2 rounded-xl border-primary/30 text-gray-900 dark:text-gray-100 outline-none">{Array.from({length:35},(_,i)=>i+2).map(b=><option key={b} className="text-black">{b}</option>)}</select></div>
          <span>→</span>
          <div><label>To Base</label><select value={toBase} onChange={e => setToBase(+e.target.value)} className="w-full glass dark:bg-gray-800/50 p-2 rounded-xl border-primary/30 text-gray-900 dark:text-gray-100 outline-none">{Array.from({length:35},(_,i)=>i+2).map(b=><option key={b} className="text-black">{b}</option>)}</select></div>
        </div>
      </div>
...
function Base64Tool() {
  ...
  const handleDecode = () => {
    try {
      setDecodedText(decodeUTF8(text))
    } catch {
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <textarea
          className="w-full h-32 bg-transparent border rounded-xl p-3 font-mono outline-none"
          value={text}
    ...
    <div>
      {courses.map((c, i) => (
        <div key={i} className="flex gap-2 mb-2">
          ...
          <select value={c.grade} onChange={e=>{ const n=[...courses]; n[i].grade=e.target.value; setCourses(n) }} className="glass dark:bg-gray-800/50 border-primary/30 rounded px-2 text-gray-900 dark:text-gray-100 outline-none"><option className="text-black">A</option><option className="text-black">B</option><option className="text-black">C</option></select>
        </div>
      ))}
...
function GraphPlot({ expression }: { expression: string }) {
  const data = useMemo(() => {
    const labels = []
    const points = []
    for (let x = -10; x <= 10; x += 0.5) {
      ...
      try {
        ...
        points.push(typeof y === 'number' ? y : null)
      } catch {
        points.push(null)
      }
    }
  ...
  return (
    <div className="glass p-4 rounded-2xl mt-6">
      <h3 className="text-sm font-semibold mb-4 text-center">Function Graph</h3>
...
function ScientificCalc() {
  ...
  const handleCalculate = () => {
    try {
      let processedExpr = expr.replace(/Math\./g, '');
      ...
      if (evalResult === Infinity || evalResult === -Infinity) {
        ...
        setResult(typeof evalResult === 'number' ? evalResult.toString() : String(evalResult));
      }
    } catch (e: any) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4">
      <input value={expr} onChange={e=>setExpr(e.target.value)} className="w-full bg-transparent border-b p-2" placeholder="e.g., sin(pi/2)" maxLength={100} />
      <div className="flex gap-2">
        <button onClick={handleCalculate} className="flex-1 bg-primary text-white px-4 py-2 rounded-xl">Calculate</button>
        <button onClick={() => setShowGraph(!showGraph)} className="flex-1 glass hover:bg-white/50 dark:hover:bg-gray-800/50 px-4 py-2 rounded-xl">
          {showGraph ? 'Hide Graph' : 'Plot Graph'}
        </button>
      </div>
    </div>
    {result && <p className="text-xl font-bold text-center">{result}</p>}
    {showGraph && expr && <GraphPlot expression={expr.replace(/Math\./g, '')} />}
  </div>
}
...
function CalculatorPage() {
  ...
  const handleButtonClick = (val: string) => {
    if (val === 'AC') {
      ...
      try {
        let processedExpr = expression.replace(/Math\./g, '');
      ...
      setExpression(prev => (prev.length < 100 ? prev + val : prev))
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="glass p-6 rounded-3xl border-2 border-primary/20 shadow-2xl space-y-6">
        ...
        {mode === 'scientific' && showGraph && expression && (
          <GraphPlot expression={expression.replace(/Math\./g, '')} />
        )}
          ...
          {mode === 'scientific' && (
            <div className="flex-1 grid grid-cols-3 gap-3">
              ...
              <button
                ...
                className="col-span-3 p-3 rounded-2xl bg-primary/10 text-primary hover:bg-primary hover:text-white transition-all font-medium"
              >
                {showGraph ? 'Hide Graph' : 'Plot Graph'}
              </button>
            </div>
    ...
    <div className="max-w-3xl mx-auto space-y-8 py-8">
      ...
      <div className="glass p-6 rounded-2xl space-y-6">
        <section>
          ...
          <p className="text-gray-600 dark:text-gray-400 leading-relaxed">
            By accessing and using EasyunitEX, you agree to comply with and be bound by these Terms of Service. If you do not agree to these terms, please do not use our services.
          </p>
        </section>
        ...
        <section>
          ...
          <p className="text-gray-600 dark:text-gray-400 leading-relaxed mb-3">
            You agree to use EasyunitEX only for lawful purposes. You are prohibited from:
          </p>
          <ul className="list-disc list-inside space-y-2 text-gray-600 dark:text-gray-400">
            <li>Attempting to disrupt the service or its infrastructure</li>
        ...
        <section>
          <h2 className="text-xl font-bold mb-3">3. Limitation of Liability</h2>
          <p className="text-gray-600 dark:text-gray-400 leading-relaxed">
            EasyunitEX is provided "as is" without warranties of any kind. We are not responsible for any inaccuracies in conversion results or any data loss resulting from the use of this tool.
          </p>
        </section>

        <section>
          <h2 className="text-xl font-bold mb-3">4. Governing Law</h2>
          <p className="text-gray-600 dark:text-gray-400 leading-relaxed">
